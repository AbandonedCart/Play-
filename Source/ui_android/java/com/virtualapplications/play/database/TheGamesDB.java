	...
	public int bulkInsert(Uri uri, ContentValues values[]) {
		...
		try {
			...
			for (ContentValues value : values) {
				...
				if (KEY_GAMEID != null) { stmt.bindString(1, KEY_GAMEID); } else { stmt.bindNull(1); }

				String KEY_TITLE = (String)(value.get(Games.KEY_TITLE));
				if (KEY_TITLE != null) { stmt.bindString(2, KEY_TITLE); } else { stmt.bindNull(2); }

				String KEY_OVERVIEW = (String)(value.get(Games.KEY_OVERVIEW));
				if (KEY_OVERVIEW != null) { stmt.bindString(3, KEY_OVERVIEW); } else { stmt.bindNull(3); }

				String KEY_SERIAL = (String)(value.get(Games.KEY_SERIAL));
				if (KEY_SERIAL != null) { stmt.bindString(4, KEY_SERIAL); } else { stmt.bindNull(4); }

				String KEY_BOXART = (String)(value.get(Games.KEY_BOXART));
				if (KEY_BOXART != null) { stmt.bindString(5, KEY_BOXART); } else { stmt.bindNull(5); }
